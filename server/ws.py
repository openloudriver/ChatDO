...
Keep it concise, neutral, and factual."""
        ...
        # Check for API intents (weather, crypto, stock) before routing to GPT/web/RAG
        from server.services.api_intents import detect_api_intent
        from server.services.api_router import ApiRouter, WeatherRequest, CryptoPriceRequest, StockQuoteRequest
        from datetime import datetime, timezone
        
        api_intent = detect_api_intent(message)
        
        if api_intent is not None:
            try:
                if api_intent.intent == "api_weather_current":
                    if not api_intent.location:
                        await websocket.send_json({
                            "type": "error",
                            "content": "Location not found in weather query",
                            "done": True
                        })
                        return
                    
                    result = await ApiRouter.get_weather(WeatherRequest(location=api_intent.location))
                    
                    # Build API message
                    api_data = {
                        "kind": "api_weather_current",
                        "location": result.location_name or result.location,
                        "temperature_c": result.temperature_c,
                        "temperature_f": result.temperature_f,
                        "condition": result.condition,
                        "provider": result.provider or result.source,
                        "lat": result.latitude or result.lat,
                        "lon": result.longitude or result.lon,
                    }
                    
                    # Build source for attribution
                    source_url = "https://api.weather.gov" if result.provider == "nws" else "https://open-meteo.com"
                    source = {
                        "id": f"api-weather-{result.provider or 'unknown'}",
                        "url": source_url,
                        "title": f"Weather data from {result.provider or result.source}",
                        "siteName": result.provider or result.source,
                        "description": f"Current weather for {result.location_name or result.location}",
                        "rank": 0,
                    }
                    
                    await websocket.send_json({
                        "type": "api_result",
                        "api_kind": "api_weather_current",
                        "data": api_data,
                        "sources": [source],
                        "model": "API",
                        "provider": result.provider or result.source,
                    ...
                    if conversation_id and project_id:
                        try:
                            ...
                            print(f"Warning: Failed to save API weather result to memory store: {e}")
                ...
                elif api_intent.intent == "api_crypto_price":
                    if not api_intent.symbol:
                        await websocket.send_json({
                            "type": "error",
                            "content": "Crypto symbol not found in query",
                            "done": True
                        })
                        return
                    
                    result = await ApiRouter.get_crypto_price(
                        CryptoPriceRequest(symbol=api_intent.symbol.lower(), vs_currency="usd")
                    )
                    ...
                    # Build source for attribution
                    source = {
                        "id": "api-crypto-coingecko",
                        "url": "https://www.coingecko.com",
                        "title": "Crypto price data from CoinGecko",
                        "siteName": "CoinGecko",
                        "description": f"Current price for {result.symbol}",
                        "rank": 0,
                    }
                    
                    await websocket.send_json({
                        "type": "api_result",
                        "api_kind": "api_crypto_price",
                        "data": api_data,
                        "sources": [source],
                        "model": "API",
                        "provider": "coingecko",
                    ...
                    if conversation_id and project_id:
                        try:
                            ...
                            print(f"Warning: Failed to save API crypto result to memory store: {e}")
                ...
                elif api_intent.intent == "api_stock_price":
                    if not api_intent.symbol:
                        await websocket.send_json({
                            "type": "error",
                            "content": "Stock symbol not found in query",
                            "done": True
                        })
                        return
                    
                    result = await ApiRouter.get_stock_quote(StockQuoteRequest(symbol=api_intent.symbol))
                    ...
                    api_data = {
                        ...
                        "trailing_dividend_rate": result.trailing_dividend_rate,
                        "trailing_dividend_yield": result.trailing_dividend_yield,
                    ...
                    # Build source for attribution
                    source = {
                        "id": "api-stock-yahoo-finance",
                        "url": "https://finance.yahoo.com",
                        "title": "Stock quote data from Yahoo Finance",
                        "siteName": "Yahoo Finance",
                        "description": f"Current quote for {result.symbol}",
                        "rank": 0,
                    }
                    
                    await websocket.send_json({
                        "type": "api_result",
                        "api_kind": "api_stock_price",
                        "data": api_data,
                        "sources": [source],
                        "model": "API",
                    ...
                    if conversation_id and project_id:
                        try:
                            ...
                            print(f"Warning: Failed to save API stock result to memory store: {e}")
                ...
                import traceback
                print(f"Error handling API intent: {e}")
                traceback.print_exc()
                await websocket.send_json({
                    "type": "error",
