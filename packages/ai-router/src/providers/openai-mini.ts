...
export const openAiGpt5MiniProvider: AiProvider = {
  id: "openai-gpt5-mini",
  label: "GPT-5 Mini",
  costTier: "cheap",
  ...
  async invoke(input: AiRouterInput): Promise<AiRouterResult> {
    ...
    // Default to gpt-5-mini if not specified
    const modelId = (input.input.systemHint as string) || "gpt-5-mini";
    ...
    // gpt-5-mini uses v1/chat/completions endpoint
    // gpt-5-mini models don't support custom temperature - only default (1)
    const response = await client.chat.completions.create(requestPayload);

    // Get the assistant message from OpenAI response
    const assistantMessage = response.choices[0]?.message;
    const content = assistantMessage?.content ?? "[openai-gpt5-mini] empty response";
